    ...
    return (
        <div className={`${style['sec']} flex-col-center`}>
            ...
            <div className={`${style['sec--panel']} flex-row-center radius-20px`}>
                <div className={`${style['sec--panel--side']} white-bg radius-5px`}>
                    {[...Array(6)].map((el, i) => (<div key={i} className={`${style['sec--panel--side--dot']} black-bg radius-circular shadow-2px`}></div>))}
                </div>
                <CSSTransition in={formShown} mountOnEnter unmountOnExit timeout={1000} classNames={{ enterActive: style['withPostMan-enter-active'], exitActive: style['withPostMan-exit-active'] }} onExited={() => setCoverShown(true)}>
                    <div className={`${style['sec--panel--body']} dark-blue-bg radius-5px flex-col-left-start`}>
                        <p className="open-sans white size-26px"><i className="bi bi-chat-right-text size-28px"></i> Let's Keep In Touch!</p>
                        <form onSubmit={formik.handleSubmit} className={`${style['sec--panel--body--form']} flex-col-left-start`}>
                            ...
                            <button className={`${style['sec--panel--body--form--btn']} inter dark-blue font-bold size-16px white-bg margin-10px-V radius-10px`} type="submit"><i className="bi bi-send"></i> Send</button>
                        </form>
                    </div>
                </CSSTransition>
                <CSSTransition in={coverShown} mountOnEnter unmountOnExit timeout={1000} classNames={{ enterActive: style['withPostMan-enter-active'], exitActive: style['withPostMan-exit-active'] }} onExited={() => setFormShown(true)}>
                    <div className={`${style['sec--panel--body']} dark-blue-bg radius-5px flex-col-center`}>
                        <p className="pt-sans white size-24px">Thank you , we will contact you soon </p>
                        <button onClick={() => setCoverShown(false)} className={`${style['sec--panel--body--form--btn']} inter dark-blue font-bold size-16px white-bg margin-10px-V radius-10px`}><i className="bi bi-envelope"></i> New Message ?</button>
                    </div>
                </CSSTransition>
            </div >
